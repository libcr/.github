...
一款用于在局域网内共享文件、文本、链接和图片的轻量级桌面应用。crShare 使用
原生 C++ 与 Chromium Views 构建，可将选定的文件夹转化为简洁的浏览器共享空间，
其他设备无需安装客户端即可使用。

主要功能：

- 支持同时共享多个文件夹，可自定义显示名称、调整顺序并分别设置上传权限，相关更改
  会实时同步到已连接的页面
- 提供基于浏览器的目录导航、排序、流式下载、拖放上传和实时进度，支持最大 16 GiB
  的文件
- Quick Share 可通过 WebSocket 按主题实时共享文本、链接、粘贴的图片和拖入的文件
- 支持可选的访问码认证、记住设备 30 天、集成二维码，以及由持久化本地证书颁发机构
  支持的 HTTPS
- 提供可搜索的传输历史，支持保留期限、隐私模式、传输详情和 CSV 导出
- HTTP 服务运行在独立进程中，原生界面和网页界面均支持多语言，并提供浅色、深色和
  跟随系统主题
